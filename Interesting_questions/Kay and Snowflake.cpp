...
int Main(){
   ...
   vector< int > centroid ( n +1 ), siz ( n +1), par( 1+n) , max_child( n +1 ) ;
   iota( all( par) , 0 ) ;
   
   auto is_centroid = [ & ] ( int v , int  u) {
	  
	  return (  ( siz[ u] - siz[ v] )*2 <=siz[ u]  && max_child[ v] *2<= siz[ u]);  
    ...
    function < void ( int , int ) > dfs = [ & ] ( int u , int parent) {
		...
		for( int i =0; i < ( int )adj[ u ].size(); i++) {
			dfs( adj[ u][ i] , u ) ;
			siz[ u] += siz[adj[ u][ i]] ;
			max_child [ u] = max( max_child[ u ], siz[ adj[ u] [ i]]) ; 
		}
	};
	
	function < void (int ) > get_centre = [ &] ( int u ) {
		...
		int tmp = centroid[ adj[ u] [ id ]] ;
		while ( ! is_centroid ( tmp , u) )
		{
			tmp = par[ tmp];
		}
		centroid [ u] = tmp ;
